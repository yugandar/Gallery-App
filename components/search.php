<?php
    ...
    if($_POST){
        ...
        //$sql_res1=mysqli_query($database,"select * from user where 	user_dob like '%$q%' or 	user_dob like '%$q%' order by user_id LIMIT 5");
        //$result=  mysql_query($sql_res) or die(mysql_errno());
        $trws= mysqli_num_rows($sql_res);
        if($trws>0){
            while($row=mysqli_fetch_array($sql_res, $sql_res1)){
